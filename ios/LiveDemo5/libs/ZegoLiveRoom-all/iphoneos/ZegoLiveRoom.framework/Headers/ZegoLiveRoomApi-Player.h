...
/**
 ...
 @param streamID 流 ID
 @return true 成功，false 失败
 @attention 调用 [self -startPlayingStream:inView:] 播放流成功以后，如果要切换流播放 View 或者停止显示流画面，调用该 API 变更
 */
...
/**
 ...
 @param streamID 流 ID
 @return true 成功，false 失败
 @attention 主播停止流推后，会通过 [ZegoRoomDelegate -onStreamUpdated:streams:roomID:] 通知房间内用户流删除，用户需要调用此 API 停止播放流
 @note 停止播放流后，注意移除相关的UI控件
 */
...
/**
 ...
 @return true 成功，false 失败
 @attention 设置为关闭后，内置扬声器和耳机均无声音输出
 */
...
/**
 ...
 @return 视频的音量值
 @attention 直播时通过此 API 获取当前音量。音量变更也会受硬件音量键的影响。
 */
...
/**
 ...
 @return true 成功，false 失败
 @attention 直播时调用此 API 可获取当前画面截图
 */
...
/**
 ...
 @attention 该设置会影响 [ZegoLivePlayerDelegate -onPlayQualityUpdate:stream:videoFPS:videoBitrate:] 的回调频率
 */
...
/**
 ...
 @attention 必须在初始化 SDK 前调用
 @note 启用外部渲染后，需要设置外部渲染回调代理对象。SDK 提供给用户外部渲染的源数据格式为 BGRA32
 */
...
/**
 ...
 @param enable true 启用音频录制，false 关闭音频录制。默认 false
 @return true 成功，false 失败
 @attention 开启音频录制，会将观众方拉流和推流的音频均录制下来
 @note 开启音频录制后，调用方需要设置音频录制回调代理对象，并通过 [ZegoLiveRoomApi (Player) -onAudioRecord:sampleRate:numOfChannels:bitDepth:type:] 获取 SDK 录制的数据
 */
...
/**
 ...
 @attention startPlay 后，以下情况下，播放端会收到该通知：1. SDK 在获取到第一帧数据后 2. 直播过程中视频宽高发生变化
 @note 从播放第一条流，到获得第一帧数据，中间可能出现一个短暂的时间差（具体时长取决于当前的网络状态），推荐在进入直播页面时加载一张预览图以提升用户体验，然后在本回调中去掉预览图
 */
...
/**
 ...
 @attention startPlay 后，该回调会被多次调用，调用周期取决于 [ZegoLiveRoomApi (Player) setPlayQualityMonitorCycle] 设置的周期
 @note 开发者可以在该回调中获取当前的视频质量数据，加以处理
 */
...
/**
 ...
 @attention 开启外部渲染，设置外部渲染代理对象成功后，SDK 通过此 API 从用户端获取 PixelBuffer 地址
 @note SDK 获取到用户指定的 PixelBuffer 后，将采集的视频源数据拷贝进去
 */
